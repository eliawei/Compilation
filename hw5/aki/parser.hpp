...
public:
    ...
    vector<pair<int,BranchLabelIndex>> inlist;
    vector<pair<int,BranchLabelIndex>> outlist;
    string registerName;
    string label;
    ...
    Node(){
        ...
        this->inlist = vector<pair<int,BranchLabelIndex>>();
        this->outlist = vector<pair<int,BranchLabelIndex>>();
        this->registerName = "";
        this->label = "";
    };
};
...
struct StringExp : public Expression{
    ...
    StringExp(Token* val){
        value = val->text.substr(1, val->text.length() - 2);
        type = "STRING";
    }
};
